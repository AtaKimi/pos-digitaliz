...
@section('content')
    ...
    <x-card>
        <div class="flex items-center justify-between mb-5">
            ...
            <div class="flex gap-4">
                ...
                <button class="flex items-center gap-2 p-3 bg-red-500 rounded-lg" data-modal-target="modal-add-category"
                    ...
                    <p class="text-sm font-semibold text-white-50">Meja Baru</p>
                </button>
            </div>
        </div>
        ...
        <x-table>
            ...
            @forelse ($desk as $item)
                <tr class="bg-white border-b">
                    ...
                    <td class="px-6 py-4">
                        <div class="flex items-center gap-2">
                            <svg width="21" height="20" viewBox="0 0 21 20" fill="none"
                                ...
                                data-modal-toggle="modal-edit-category-{{ $item->id }}">
                                <path
                    ...
                    {{-- MODAL Edit MEJA --}}
                    <div id="modal-edit-category-{{ $item->id }}" tabindex="-1"
                        ...
                        <div class="relative w-full max-w-md max-h-full">
                            <div class="flex flex-col gap-10 relative bg-white-50 rounded-xl shadow p-10 w-[500px]">
                                ...
                                <form method="POST" action="{{ route('tenant-desk-update', [$tenant, $item->id]) }}">
                                    @csrf
                                    @method('put')
                                    <div class="flex flex-col">
                                        <label for="text" class="block mb-2 text-sm font-medium">
                                            Nama Meja</label>
                                        <input type="text" id="text" name="name" value="{{ $item->name }}"
        ...
        {{ $desk->links('vendor.pagination.tenant') }}
    </x-card>
